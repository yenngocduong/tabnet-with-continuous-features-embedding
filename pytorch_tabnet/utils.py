...
def create_explain_matrix(input_dim, cat_emb_dim, cat_idxs, con_idxs, con_emb_dim, post_embed_dim):
    """
    ...
    if isinstance(con_emb_dim, int):
        con_emb_dims = [con_emb_dim] * len(con_idxs)
    else:
        con_emb_dims = con_emb_dim
    ...
    for idx in range(input_dim):
        if idx in cat_idxs:
            embedding_dim.append(cat_emb_dims[cat_idx_track])
            cat_idx_track += 1
        elif idx in con_idxs:
            embedding_dim.append(con_emb_dims[con_idx_track])
            con_idx_track += 1
        else:
            print("With i = ", idx, " Not in cat_idx and con_idx")
    print("Input dim =", input_dim)
    print("Length of embedding_dim =", len(embedding_dim))
    #print("Input dim = ", input_dim)
    #print("Cat index = ", cat_idxs)
    #print("Con index = ", con_idxs)
    #print("Cat emb dim = ", cat_emb_dims)
    #print("Con emb dim = ", con_emb_dims)

    indices_trick = []
    acc_emb = 0
    for i in range(input_dim):
        indices_trick.append(
            range(acc_emb, acc_emb + embedding_dim[i])
        )
        acc_emb += embedding_dim[i]
    #print("Shape of indices trick = ", len(indices_trick), "x", len(indices_trick[0]))
    ### END CODE HERE #########
