...
class TabNetEncoder(torch.nn.Module):
    def __init__(
        ...
        #self.samples_transformers = torch.nn.ModuleList()
        ...
        for step in range(n_steps):
            '''
            ##### START CODE HERE #########
            samplestransformer = MultiheadAttention(
                sample_dim=n_a,
                embed_size=emb_dim,
                heads=heads,
                dropout=dropout,
            ...
            ###self.samples_transformers.append(samplestransformer)
    ...
    def forward(self, x, prior=None):
        ...
        M_loss = 0
        '''
        for i in range(self.step_att):
            x = x.unsqueeze(0)
            # >> Shape of att: (1, b, n*d)
            x = self.loop_attention[i](x, x, x, mask=None)
            # >> This is multi-head self attention.
            # >> Shape of att: (1, b, n*d)
            x = x.squeeze(0)
            # shape of att: (b, n*d)
            # >> b is batch size, n is number of features, d is embedding dimension
        '''
        att = self.initial_splitter(x)[:, self.n_d:]
        # >> This is a Feature Transformer
        #print("Shape of att:", att.shape)
        #print("post_emb_dim:", self.input_dim)
        steps_output = []
        # >> Record the output of every step
        ...
        for step in range(self.n_steps):
            ...
            att = att.unsqueeze(0)
            # >> Shape of att: (1, b, n*d)
            att = self.samples_transformers[step](att, att, att, mask=None)
            # >> This is multi-head self attention.
            # >> Shape of att: (1, b, n*d)
            att = att.squeeze(0)
            '''
            ...
            # >> M is the mask, depends on prior, and output of previous block

            # Obtain the mask by attentive transformer
            M_loss += torch.mean(
            ...
            # >> Prior (P) depends on the previous step mask and previous prior

            # output
            masked_x = torch.mul(M, x)
            # >> compute the masked x by multiply x with the mask

            # Apply feature transformer to reconstruct the features
            ...
            # >> Shape of att: (1, b, n*d)
            samplestransformer = MultiheadAttention(
                sample_dim=out.shape[1],
                embed_size=self.emb_dim,
                heads=self.heads,
                dropout=self.dropout,
                forward_expansion=self.forward_expansion
            )
            out = samplestransformer(out, out, out, mask=None)
            # >> This is multi-head self attention.
            # >> Shape of att: (1, b, n*d)
            out = out.squeeze(0)
            '''
        ...
        M_loss /= self.n_steps  # compute the average loss
    ...
    def forward_masks(self, x):
        ...
        for step in range(self.n_steps):
            ...
            # the shape of attention block now is (b, n*d)
            # >> b is batch size, n is number of features, d is embedding dimension
            att = att.unsqueeze(0)
            # >> Shape of att: (1, b, n*d)
            att = self.samples_transformers[step](att, att, att, mask=None)
...
class TabNetPretraining(torch.nn.Module):
    def __init__(
        ...
        self.virtual_batch_size = virtual_batch_size
        print("In TabNet Pretraining >> before embedder, con_idxs = ", con_idxs)

        self.embedder = EmbeddingGenerator2(input_dim = input_dim, cat_dims=cat_dims, cat_idxs=cat_idxs, cat_emb_dim=cat_emb_dim, con_idxs=con_idxs, con_emb_dim=con_emb_dim)
        #self.embedder = EmbeddingGenerator(input_dim=input_dim, cat_dims=cat_dims, cat_idxs=cat_idxs, cat_emb_dim=cat_emb_dim)
...
class TabNet(torch.nn.Module):
    def __init__(
        ...
        #print("In TabNet, con_idxs=", con_idxs)
    ...
    def forward(self, x):
        ...
        #print("X:")
        #print(x)
        #print("Shape of X:", len(x), "x", len(x[0]))
        x = self.embedder(x)
        #print("Shape of x after embedding: ", x.shape)
        x = self.tabnet(x)
        #print("x after call tabnetnoembedding: ", x)
        return x
...
class DotAttention(nn.Module):
    def __init__(self, input_dim, embed_size, heads):
        super(DotAttention, self).__init__()
        self.embed_size = input_dim
        # embed_size is the size wanted after convert to keys, queries, values
        self.input_dim = input_dim
        # input dim is the post_embeded_dim
        #print("embed_size = ", embed_size)
        # embed_size = 512 for example
        #self.heads = 2
        self.head_dim = input_dim
        # this one, I just want to make one head, or no multi-head attention, just self-attention
        '''''
        assert (
        ...
        '''
        #print()
        #print("Changing head_dim equal to embed_size, only perform self-attention")
        self.values = nn.Linear(self.head_dim, self.head_dim, bias=False)
        self.keys = nn.Linear(self.head_dim, self.head_dim, bias=False)
        self.queries = nn.Linear(self.head_dim, self.head_dim, bias=False)
        self.fc_out = nn.Linear(self.head_dim, self.head_dim)
    ...
    def forward(self, values, keys, query, mask):
        ...
        #query = query.reshape(N, query_len, self.head_dim)
        #(1, 1024, 1, 8)
        ...
        # attention shape: (N, query_len, key_len)
        # values shape: (N, value_len, heads_dim)
        # out after matrix multiply: (N, query_len, head_dim), then
...
class MultiheadAttention(nn.Module):
    def __init__(self, sample_dim, embed_size, heads, dropout, forward_expansion):
        super(MultiheadAttention, self).__init__()
        self.attention = DotAttention(input_dim=sample_dim, embed_size=embed_size, heads=heads)
        self.norm1 = nn.LayerNorm(sample_dim)
        self.norm2 = nn.LayerNorm(sample_dim)

        self.feed_forward = nn.Sequential(
            nn.Linear(in_features=sample_dim, out_features=forward_expansion * sample_dim),
            nn.ReLU(),
            nn.Linear(in_features=forward_expansion * sample_dim, out_features=sample_dim),
        )
...
class EmbeddingGenerator2(torch.nn.Module):
    """
    Classical embeddings generator
    """

    def __init__(self, input_dim, cat_dims, cat_idxs, con_idxs, cat_emb_dim = 512, con_emb_dim = 512):
        """This is an embedding module for an entire set of features
        ...
        cat_emb_dim : int or list of int
            ...
            If int, the same embedding dimension will be used for all categorical features
        """
        super(EmbeddingGenerator2, self).__init__()

        self.input_dim = input_dim
        if cat_dims == [] and cat_idxs == [] and con_idxs == []:
            self.skip_embedding = True
            self.post_embed_dim = input_dim
            return
        elif len(cat_dims) != len(cat_idxs):
        ...
        # If the embedding dimension is an interget, we apply this dimension  to all the features.
        # We make this interger into a list of the same number
        if isinstance(cat_emb_dim, int):
            self.cat_emb_dims = [cat_emb_dim] * len(cat_idxs)
        else:
            self.cat_emb_dims = cat_emb_dim

        if isinstance(con_emb_dim, int):
            #print("Con_idxs:", con_idxs)
            self.con_emb_dims = [con_emb_dim] * len(con_idxs)
        else:
            self.con_emb_dims = con_emb_dim
        ...
        if len(self.cat_emb_dims) != len(cat_dims):
            msg = f"""cat_emb_dim and cat_dims must be lists of same length, got {len(self.cat_emb_dims)}
                      and {len(cat_dims)}"""
            raise ValueError(msg)
        if len(self.con_emb_dims) != len(con_idxs):
            msg = f"""con_emb_dim and con_idxs must be lists of same length, got {len(self.con_emb_dims)}
                      and {len(con_idxs)}"""
            raise ValueError(msg)


        # After embedding, categorical value change from len(cat_emb_dims) to sum(cat_emb_dims), continuous value change
        self.post_embed_dim = int(
            np.sum(self.cat_emb_dims) + np.sum(self.con_emb_dims)
        )

        self.embedding_dim_info = []
        for idx in range(self.input_dim):
            if idx in cat_idxs:
                self.embedding_dim_info.append((cat_dims[cat_idxs.index(idx)], self.cat_emb_dims[cat_idxs.index(idx)], 1))
            if idx in con_idxs:
                self.embedding_dim_info.append((1, self.con_emb_dims[con_idxs.index(idx)], 0))


        self.embeddings = torch.nn.ModuleList()
        for in_dim, emb_dim, is_cat in self.embedding_dim_info:
            if is_cat:
                self.embeddings.append(torch.nn.Embedding(num_embeddings=in_dim, embedding_dim=emb_dim))
            else:
                self.embeddings.append(torch.nn.Linear(in_dim, emb_dim))
    def forward(self, x):
        """
        Apply embeddings to inputs
        Inputs should be (batch_size, input_dim)
        ...
        cols = []
        count_con_relu = 0
        for idx, (in_dim, emb_dim, is_cat) in enumerate(self.embedding_dim_info):
            if is_cat:
                temp_col = self.embeddings[idx](x[:, idx].long())
                cols.append(temp_col)
            else:
                temp_col = self.embeddings[idx](x[:, idx].reshape(-1, 1))
                cols.append(temp_col)
        post_embeddings = torch.cat(cols, dim=1)
        return post_embeddings
